    ...
    <div className="min-h-screen flex flex-col bg-white text-slate-900">
      <header className="border-b border-slate-100 bg-white/95 backdrop-blur-sm sticky top-0 z-50">
        <nav className="container h-16 flex items-center justify-between">
          <Link to="/" className="text-xl font-bold bg-gradient-to-r from-brand to-blue-600 bg-clip-text text-transparent">
          ...
          <div className="hidden md:flex items-center gap-8">
            <Link 
              to="/analyze" 
              className="text-slate-700 hover:text-brand font-medium transition-colors"
            >
              Analyze Essay
            </Link>
            {isAuthenticated ? (
              <>
                ...
                <button
                  onClick={logout}
                  className="px-4 py-2 border border-slate-200 text-slate-700 font-medium rounded-lg hover:border-brand hover:text-brand transition-colors"
                >
                  Logout
                </button>
              </>
              ...
              <Link 
                to="/login" 
                className="px-4 py-2 bg-brand text-white font-medium rounded-lg hover:bg-brand/90 transition-colors"
              >
                Login
              </Link>
            )}
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-slate-100 bg-white">
            <div className="container py-4 space-y-4">
              <Link 
                to="/analyze" 
                className="block text-slate-700 hover:text-brand font-medium transition-colors"
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <button
                    ...
                    className="block w-full text-left px-4 py-2 border border-slate-200 text-slate-700 font-medium rounded-lg hover:border-brand hover:text-brand transition-colors"
                  >
                    Logout
                  </button>
                </>
                ...
                <Link 
                  to="/login" 
                  className="block px-4 py-2 bg-brand text-white font-medium rounded-lg hover:bg-brand/90 transition-colors text-center"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Login
                </Link>
      ...
      <footer className="border-t border-slate-100 bg-slate-50">
        <div className="container py-12">
          <div className="grid md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-slate-600">
                <li><a href="/analyze" className="hover:text-brand transition-colors">Analyze Essay</a></li>
          ...
          <div className="border-t border-slate-200 mt-8 pt-8 text-center text-slate-500">
            © {new Date().getFullYear()} IELTS Band Estimator. All rights reserved. Improve your IELTS Writing faster.
          </div>
        </div>
      </footer>
    </div>
